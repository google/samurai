...
class FourierEmbedding(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.random_offsets = tf.zeros_like(self.scales)[:, None]
        if random_offsets:
            rng = np.random.default_rng(1337)  # Not even sorry for the seed
            random_offsets = tf.convert_to_tensor(
                rng.normal(size=(self.num_frequencies, input_dim), scale=0.1),
                tf.float32,
            )
            self.random_offsets = random_offsets * self.scales[:, None]
    ...
    def call(self, x):
        ...
        xb = tf.reshape(
            x[..., None, :] * self.scales[:, None] + self.random_offsets,
...
def volumetric_rendering(
    ...
    # sample yet.
    # [N_rays, N_samples]
    accum_prod = tf.math.cumprod(1.0 - alpha + eps, axis=-1, exclusive=True)
    ...
    payload_raymarched = {
        k: tf.reduce_sum(math_utils.saturate(weights[..., None]) * v, axis=-2)
        for k, v in payload_dict.items()
    }

    payload_raymarched.update({"individual_" + k: v for k, v in payload_dict.items()})
    ...
    payload_raymarched["sigmas"] = sigma
...
def add_gaussian_noise(raw, noise_std, randomized):
    if (noise_std is not None) and noise_std > 0 and randomized:
        return raw + tf.random.normal(tf.shape(raw), dtype=raw.dtype) * noise_std
    else:
        return raw
