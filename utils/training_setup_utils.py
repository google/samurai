...
def setup_parser():
    ...
    parser.add_argument("--tpu", type=str, help="Define the TPU worker address")

    parser.add_argument(
        "--debug", action="store_true", help="Activates NaN and Inf debugging."
    )
    parser.add_argument(
        "--profile", action="store_true", help="Activates NaN and Inf debugging."
    )
...
def get_num_tpus():
    ...
    return len(tf.config.list_physical_devices("TPU"))


def get_num_replicas(args):
...
class StateRestoration:
    ...
    def restore(self, step: Optional[int] = None) -> int:
        if step is None:
            ...
            def name_to_step(name):
                return int(
                    name.replace(self.to_watch[0].name + "_", "").replace(
                        ".pickle", ""
                    ),
                )
    ...
    def build_save_path(cls, args, prefix, i):
        return os.path.join(
            args.basedir, args.expname, "{}_{:06d}.pickle".format(prefix, i)
        )
    ...
    def save_weights(cls, args, net, prefix, i):
        ...
        # weights = np.array(net.get_weights(), dtype=object)
    ...
    def save_variable(cls, args, variable, prefix, i):
        path = StateRestoration.build_save_path(args, prefix, i)
        with open(path, "wb") as handle:
            pickle.dump(variable.numpy(), handle, protocol=pickle.HIGHEST_PROTOCOL)
...
class SetupDirectory:
    def __init__(
        ...
        self.logdir = os.path.join(args.basedir, "summaries", args.expname)
        ...
        if copy_files:
            ...
            all_files = find_files_to_copy(os.path.basename(main_script))
